...
#define PCAP_HEADER_LENGTH (42)
#define GT_PACKET_LENGTH   (1302)
...
Input::Input(std::string pcap_path) {
  pcapFilePath = pcap_path;

  if ((pcap_ = pcap_open_offline(pcap_path.c_str(), pcapErrorBuffer)) == NULL) {
...
//          1 - gps
//          -1 - error
int Input::getPacket(DataPacket *pkt) {
...
int Input::getPacketFromPcap(DataPacket *pkt) {
  ...
  if (1 == ret) {
    memcpy(&pkt->data[0], pcapData + PCAP_HEADER_LENGTH, GT_PACKET_LENGTH);
    pkt->size = GT_PACKET_LENGTH;

    return 0;
  } else if (-2 == ret) { // at the end of pcap file
    pcap_close(pcap_);
